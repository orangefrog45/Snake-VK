...
namespace SNAKE {
	class BLAS {
	public:
		void GenerateFromMeshData(MeshDataAsset* p_mesh_data) {
			...
			as_geom_info.geometry.triangles.vertexData = p_mesh_data->position_buf.GetDeviceAddress();
			as_geom_info.geometry.triangles.indexData = p_mesh_data->index_buf.GetDeviceAddress();
			as_geom_info.geometry.triangles.vertexFormat = vk::Format::eR32G32B32Sfloat;
			as_geom_info.geometry.triangles.maxVertex = p_mesh_data->num_vertices;
			...
			vk::AccelerationStructureBuildSizesInfoKHR as_build_sizes_info = logical_device.device->getAccelerationStructureBuildSizesKHR(
				vk::AccelerationStructureBuildTypeKHR::eDevice, as_build_geom_info, p_mesh_data->num_indices / 3);
			...
			as_build_range_info.primitiveCount = p_mesh_data->num_indices / 3;
		...
		vk::AccelerationStructureInstanceKHR GenerateInstance(const TransformComponent& comp) {
	...
	public:
		void InitTLAS(Scene& scene, FrameInFlightIndex idx) {
			...
			for (auto [entity, mesh] : mesh_view.each()) {
				instances.push_back(GetOrCreateBlasFromMeshData(mesh.mesh_asset->data.get()).GenerateInstance(reg.get<TransformComponent>(entity)));
			}
		...
		BLAS& GetOrCreateBlasFromMeshData(MeshDataAsset* p_data) {
			if (!m_blas_map.contains(p_data)) {
				m_blas_map[p_data].GenerateFromMeshData(p_data);
			}

			return m_blas_map[p_data];
		}
		...
		void InitDescriptorBuffers(Image2D& output_image) {
			rt_descriptor_set_spec = std::make_shared<DescriptorSetSpec>();
			rt_descriptor_set_spec->AddDescriptor(0, vk::DescriptorType::eAccelerationStructureKHR, vk::ShaderStageFlagBits::eRaygenKHR)
		...
		void Init(Scene& scene, Image2D& output_image, vk::DescriptorSetLayout common_ubo_set_layout) {
			...
			InitDescriptorBuffers(output_image);
