...
namespace SNAKE {
	glm::mat3 ExtraMath::Scale2D(float x, float y) {
	...
	glm::mat3 ExtraMath::Rotate3D(float rotX, float rotY, float rotZ) {
		glm::quat quat(glm::radians(glm::vec3(rotX, rotY, rotZ)));
		return glm::mat3_cast(quat);
	}

	glm::mat3 ExtraMath::Scale3D(float scaleX, float scaleY, float scaleZ) {
		return glm::mat3{
			scaleX, 0.0f, 0.0f,
			0.0f, scaleY, 0.0f,
			0.0f, 0.0f, scaleZ,
	...
	glm::mat4x4 ExtraMath::Translation3D(float tranX, float tranY, float tranZ) {
