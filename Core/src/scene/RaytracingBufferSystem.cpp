...
void RaytracingInstanceBufferSystem::OnSystemAdd() {
	m_mesh_event_listener.callback = [this](Event const* p_event) {
		...
		if (p_casted->event_type == ComponentEventType::UPDATED)
			m_instances_to_update.push_back(std::make_pair(p_casted->p_component->GetEntity()->GetEnttHandle(), 0));
		else if (p_casted->event_type == ComponentEventType::ADDED) {
			p_casted->p_component->GetEntity()->AddComponent<RaytracingInstanceBufferIdxComponent>()->idx = m_current_buffer_idx;
			m_instances_to_update.push_back(std::make_pair(p_casted->p_component->GetEntity()->GetEnttHandle(), 0));

			// Allocate space for submeshes
			m_current_buffer_idx += p_casted->p_component->mesh_asset->data->submeshes.size();
		}
		};
...
void RaytracingInstanceBufferSystem::UpdateBuffer(FrameInFlightIndex idx) {
	...
	for (size_t i = 0; i < m_instances_to_update.size(); i++) {
		...
		auto& mesh_buffer_entry_data = mesh_buffer_manager.GetEntryData(mesh_comp.mesh_asset->data.get());
		...
		for (size_t j = 0; j < mesh_comp.mesh_asset->data->submeshes.size(); j++) {
			auto& mat = mesh_comp.materials[mesh_comp.mesh_asset->data->submeshes[j].material_index];
			instance_data.material_idx = mat->GetGlobalBufferIndex();
			instance_data.mesh_buffer_vertex_offset = mesh_buffer_entry_data.data_start_vertex_idx + mesh_comp.mesh_asset->data->submeshes[j].base_vertex;
			instance_data.mesh_buffer_index_offset = mesh_buffer_entry_data.data_start_indices_idx + mesh_comp.mesh_asset->data->submeshes[j].base_index;
