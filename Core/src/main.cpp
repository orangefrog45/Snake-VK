...
namespace SNAKE {
	...
	public:
		void Init(const char* app_name) {
			...
			// Create common descriptor set layout for UBO object used in CreateGraphicsPipeline and CreateDescriptorSets
		...
		void CreateWindowSurface() {
			...
			auto res = vk::Result(glfwCreateWindowSurface(m_instance.get(), window.p_window, nullptr, &surface_temp));
			m_surface = vk::UniqueSurfaceKHR{ surface_temp };
		...
		void DrawFrame() {
			...
			auto wait_semaphores = util::array<vk::Semaphore>(*m_image_avail_semaphores[m_current_frame]); // Which semaphores to wait on
			...
			auto signal_semaphores = util::array<vk::Semaphore>(*m_render_finished_semaphores[m_current_frame]);
			...
			auto swapchains = util::array<vk::SwapchainKHR>(*m_swapchain);
		...
		void CreateDescriptorSetLayout() {
			...
			// More descriptors can be added at other bindings
		...
		void CreateUniformBuffers() {
			...
			for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
				...
				SNK_CHECK_VK_RESULT(
					m_device->mapMemory(*m_uniform_buffers_memory[i], 0, buffer_size, vk::MemoryMapFlags{ 0 }, &m_uniform_buffers_mapped[i]),
					"UniformBuffers mapMemory"
				);
			}
		}
		...
		void UpdateUniformBuffer(uint32_t current_image) {
			...
			ubo.proj = glm::perspective(glm::radians(45.f), m_swapchain_extent.width / (float)m_swapchain_extent.height, 0.01f, 10.f);
		...
		void CreateDescriptorPool() {
			...
			auto [res, pool] = m_device->createDescriptorPoolUnique(pool_info);
			SNK_CHECK_VK_RESULT(res, "Descriptor pool created");
		...
		void CreateDescriptorSets() {
			...
			auto [res, sets] = m_device->allocateDescriptorSetsUnique(alloc_info);
			SNK_CHECK_VK_RESULT(res, "Descriptor set allocation");
			...
			for (size_t i = 0; i < sets.size(); i++) {
				...
				descriptor_write.pBufferInfo = &buffer_info; // Leave pImageInfo and pTexelBufferView fields empty as this descriptor refers to just buffer data
